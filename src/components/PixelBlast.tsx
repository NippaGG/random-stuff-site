...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(([entry]) => {
      ...
      if (!wasVisible && entry.isIntersecting) {
        ...
        if (state && !pausedRef.current && state.raf === undefined && state.animate) {
  ...
  // Main WebGL Initialization & Lifecycle Effect
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    ...
    if (initProps.transparent) renderer.setClearAlpha(0);
    else renderer.setClearColor(0x000000, 1);

    const uniforms = {
      uResolution: { value: new THREE.Vector2(0, 0) },
      uTime: { value: 0 },
      uColor: { value: new THREE.Color(initProps.color) },
      uClickPos: {
        value: Array.from({ length: MAX_CLICKS }, () => new THREE.Vector2(-1, -1)),
      },
      uClickTimes: { value: new Float32Array(MAX_CLICKS) },
      uShapeType: { value: SHAPE_MAP[initProps.variant] ?? 0 },
      uPixelSize: { value: initProps.pixelSize * renderer.getPixelRatio() },
      uScale: { value: initProps.patternScale },
      uDensity: { value: initProps.patternDensity },
      uPixelJitter: { value: initProps.pixelSizeJitter },
      uEnableRipples: { value: initProps.enableRipples ? 1 : 0 },
      uRippleSpeed: { value: initProps.rippleSpeed },
      uRippleThickness: { value: initProps.rippleThickness },
      uRippleIntensity: { value: initProps.rippleIntensityScale },
      uEdgeFade: { value: initProps.edgeFade },
    };
    ...
    const setSize = () => {
      ...
      uniforms.uPixelSize.value = initProps.pixelSize * renderer.getPixelRatio();
    };
    setSize();
    ...
    if (liquid) {
      ...
      touch.radiusScale = initProps.liquidRadius;
      composer = new EffectComposer(renderer);
      const renderPass = new RenderPass(scene, camera);
      liquidEffect = createLiquidEffect(touch.texture, {
        strength: initProps.liquidStrength,
        freq: initProps.liquidWobbleSpeed,
      });
    ...
    const animate = () => {
      ...
      if (autoPauseOffscreenRef.current && !visibilityRef.current.visible) {
        // Halt RAF when offscreen; observer will resume on re-entry
        if (threeRef.current) threeRef.current.raf = undefined;
        return;
      }
  ...
  // Dynamic Uniforms and Props Update Effect
  useEffect(() => {
